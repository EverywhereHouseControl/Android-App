...
public class CreateNewEventActivity extends Activity 
{
	...
	protected void onCreate( Bundle savedInstanceState ) 
	{
		...
		try 
		{
			...
			ArrayAdapter<String> spinnerAdapter = new ArrayAdapter<String>( this, R.layout.spinner_simple_data,
		...
		_itemList.setOnItemSelectedListener( new OnItemSelectedListener() 
		{
			...
			{				
				if ( position != 0 ) 
				{
					...
					int _service2 = parent.getItemAtPosition(position ).toString().indexOf( "-" );
					int _service1 = parent.getItemAtPosition( position ).toString().length();
					String _service = parent.getItemAtPosition( position ).toString().substring( _service2 + 2, _service1 );
					_actions = setActionList( _service );

					ArrayAdapter<String> _spinnerActionAdapter = new ArrayAdapter<String>( getApplicationContext(),
																						R.layout.spinner_simple_data, _actions );
					_actionList.setAdapter( _spinnerActionAdapter );
					_actionList.setSelection( 0 );

					_actionList.setOnItemSelectedListener( new OnItemSelectedListener() 
					{
						...
						public void onItemSelected(	AdapterView<?> parent, View view, int position, long id ) 
						{
							if ( ( position != 0 ) && ( _serviceSelected ) )
							{
								...
								_data = _actions.get( position ).toUpperCase();
							}
							else
							{
								_data = _actions.get( position ).toUpperCase();
		...
		_acceptButton.setOnClickListener( new OnClickListener() 
		{
			public void onClick( View v ) 
			{
				...
				if (_textChanged == false)
					Toast.makeText(getApplicationContext(), "Insert a name", Toast.LENGTH_LONG ).show();
					...
					Toast.makeText(getApplicationContext(), "Select a time", Toast.LENGTH_LONG ).show();
				else if (_actions.get(0).toString().equals( "No actions for this service" ) )
					Toast.makeText(getApplicationContext(), "No valid service to save an event", Toast.LENGTH_LONG ).show();
				else 
				{
					_selectedService = parseService( _itemList.getSelectedItem().toString() );
...
//									.toString());
	...
	protected Dialog onCreateDialog( int id ) 
	{
		...
		{
		case TIME_DIALOG:
			return new TimePickerDialog( this, timerPickerListener, _hourSelected, _minuteSelected, false );
		case DATE_DIALOG:			
			return new DatePickerDialog(this, datePickerListener, _yearSelected, _monthSelected, _daySelected );
		}
	...
	private DatePickerDialog.OnDateSetListener datePickerListener = new DatePickerDialog.OnDateSetListener() 
	{
		...
		{			
			monthOfYear++;
			_dateSelected.setText("Selected date: " + dayOfMonth + "-" 	+ monthOfYear + "-" + year );
			_yearSelected = year;
	...
	private TimePickerDialog.OnTimeSetListener timerPickerListener = new TimePickerDialog.OnTimeSetListener() 
	{
		...
		{
			String _hourOfDayStr = null; String minuteStr = null; 
			if ( hourOfDay < 10 )
			...
			_timeSelected.setText("Selected time: " + _hourOfDayStr + ":" + minuteStr);
    ...
    {    	
		...
		protected String doInBackground( String... arg0 ) 
		{
			...
			{		             
				...
				_parametros.add(_dateSelected.getText().toString().subSequence( _dateSelected.getText().toString().indexOf( ':' ) + 1, _dateSelected.getText().toString().length() ).toString() 
								+
								_timeSelected.getText().toString().subSequence( _dateSelected.getText().toString().indexOf( ':' ) + 1, _timeSelected.getText().toString().length() ).toString() );
